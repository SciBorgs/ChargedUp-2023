...
public class PlacementKinematics {
    ...
    private DMatrix3x3 M(State state) {
        ...
        DMatrix3x3 M_LU = new DMatrix3x3(0, 0, 0,
            arm.length * claw.radius * claw.mass * Math.cos(state.angleClaw - state.angleArm), 0, 0,
            (arm.length * claw.mass + arm.radius * arm.mass) * Math.cos(state.angleArm), claw.radius * claw.mass * Math.cos(state.angleClaw), 0
        );
    ...
    private DMatrix3x3 C(State state) {
        ...
        C.set(0, 1,  state.angleClawDot * arm.length * claw.radius * claw.mass * Math.sin(state.angleArm - state.angleClaw));
        C.set(1, 0,  state.angleArmDot  * arm.length * claw.radius * claw.mass * Math.sin(state.angleClaw - state.angleArm));
        C.set(2, 0, -state.angleArmDot  * (arm.length * claw.mass + arm.radius * arm.mass) * Math.sin(state.angleArm));
