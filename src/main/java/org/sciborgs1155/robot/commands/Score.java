...
public class Score {
    ...
    public final Command score(GamePiece gamePiece, ScoringHeight height, Side side) {
        ...
        return placement.toState(scoringState(gamePiece, height, side)).andThen(
               intake.start(true)).andThen(Commands.waitSeconds(3)).andThen(intake.stop());
    }
    ...
    private final Pose2d closestScoringPoint(Side side, Color color) {
        ...
        if (side == Side.BACK) { rotationDeg = (rotationDeg + 180) % 360; }
