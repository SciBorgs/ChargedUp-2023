...
public class Placement extends SubsystemBase implements Loggable, AutoCloseable {
  ...
  public Command drive(CommandJoystick left, CommandJoystick right) {
    return run(() -> {
      double height = MathUtil.interpolate(Constants.Dimensions.ELEVATOR_MIN_HEIGHT, Constants.Dimensions.ELEVATOR_MAX_HEIGHT, (left.getZ() - right.getZ()) / 2);
      double elbowAngle = MathUtil.interpolate(Constants.Dimensions.ELBOW_MIN_ANGLE, Constants.Dimensions.ELBOW_MAX_ANGLE, (1 - left.getY()) / 2);
      double wristAngle = MathUtil.interpolate(Constants.Dimensions.WRIST_MIN_ANGLE, Constants.Dimensions.WRIST_MAX_ANGLE, (1 - right.getY()) / 2);
